...
def summary():
    ...
    html_parts = [
        '<table border="1">',
        '<tr><th>ID</th><th>Company</th><th>Amount</th><th>Payment Dates</th><th>Status</th><th>Due Date</th></tr>'
    ]
    for row in data:
        payment_date = row[3] if row[3] else 'NA'
        formatted_date = format_date_mmddyyyy(payment_date)
        html_parts.append(
            f'<tr><td>{row[0]}</td><td>{row[1]}</td><td>{row[2]}</td><td>{formatted_date}</td><td>{row[4]}</td><td>{row[5]}</td></tr>')
    html_parts.append(f'<tr><td colspan="5"><strong>Total Amount:</strong></td><td>&dollar;{total_amount}</td></tr>')
    html_parts.append(f'<tr><td colspan="5"><strong>Tax Rate:</strong></td><td>{tax_rate * 100}%</td></tr>')
    html_parts.append(f'<tr><td colspan="5"><strong>Tax Due:</strong></td><td>&dollar;{tax_due}</td></tr>')
